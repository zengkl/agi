...
def rayshooting(A, b, x0, verbose=False):
    ...
    while AB.rows _____ n:
        ...
        K = [i for i in range(_____) if i not in G and (A[i, :]*d)[0] _____ 0]
        if not K: # if K is empty
            d = _____
            K = [i for i in range(m) if i not in G and (A[i, :]*d)[0] > 0]

        # compute epsilon
        L = [((b[k] - (_____)[0])/((_____)[0]), k) for k in K]
        epsilon = min(L)[0]
        ...
        C = [i for i in range(m) if i not in G and _____]
